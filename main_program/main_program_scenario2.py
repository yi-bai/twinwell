...
revised on 2018/8/5
only one dic_graph is used. only one type of lane. As there are two lanes for each link, the jam density is doubled.
bus and car have the same priority.
lane speeds are set all of 40km
'''
...
outfile=open(r"D:\03Work during PD\31ERP2_in_COI\meso_by_python\output_S2.csv","ab")#%%%%%%%%%%%%%%%%%%%%%for checking the results
...
def updatelanes(curr_timestamp):
    ...
    for lane_id in dic_lanes.keys():
        ...
        dic_graph[id1][id2]['density']=density
        dic_graph[id1][id2]['speed']=speed
        dic_graph[id1][id2]['travel-time']=travel_time
        dic_graph[id1][id2]['dic-speeds'][curr_timestamp]=speed
        dic_graph[id1][id2]['dic-counts'][curr_timestamp]=copy.deepcopy(dic_lanes[lane_id]['counts'])
        dic_graph[id1][id2]['dic-densities'][curr_timestamp]=density  
...
# set jam density used in this research; as two lanes are treated as a whole, double the jam density
jamdensity=124*2# 200 pcu/mile=124 pcu/km. from:https://lost-contact.mit.edu/afs/eos.ncsu.edu/info/ce400_info/www2/flow1.html
...
dic_nodes={}
dic_graph={}# all lanes for route search
dic_lanes={}
...
dic_graph,dic_lanes=ReadLanes2(infile_lane,dic_graph,dic_lanes)

# re-set the free speed as 40km/h in both dictionaries of lanes
for lane_id in dic_lanes:
    dic_lanes[lane_id]['free-speed']=40
    nodeid1=dic_lanes[lane_id]['node-id1']
    nodeid2=dic_lanes[lane_id]['node-id2']
    dic_graph[nodeid1][nodeid2]['free-speed']=40

# calculate the length of lanes
dic_graph =LengthCal(dic_graph, dic_nodes)
...
for item in dic_lanes:
    ...
    writer.writerow(["dic-graph", dic_graph[id1][id2]['lane-id'],dic_graph[id1][id2]])
...
dic_vehicles,dic_gen_time_vehids=vehicle_gen(OD_matrices, 'uniform',vehicle_id,dic_graph,median_value_time,dic_nodes,dic_vehicles,dic_gen_time_vehids)
    ...
    if dic_gen_time_vehids.has_key(curr_timestamp):# this time stamp generates vehicles           
        ...
        for veh_id in list_gen_vehids:# not generated at the initial time of simulation; need update their best route at first
            if curr_timestamp<>simu_start_time_stamp:
                ...
                D_low,P_low=shortestPathNode(dic_graph, O, D)
                best_route_lanes=convert_ppath_to_pathids(P_low,dic_graph,O,D)
                time_budget=D_low[D]
...
outfile_lane_features=open(r'D:\03Work during PD\31ERP2_in_COI\meso_by_python\outfile_lane_features_S2.csv',"ab")
writer_lane_features=csv.writer(outfile_lane_features)

outfile_veh_features=open(r'D:\03Work during PD\31ERP2_in_COI\meso_by_python\outfile_veh_features_S2.csv',"ab")
